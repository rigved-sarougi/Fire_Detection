...
@st.cache_resource
def load_model():
    model = torch.hub.load('ultralytics/yolov5', 'custom', path="weights/yolov8n.pt", force_reload=True)
    return model

demo_img = "fire.9.png"  # Replace with a demo image containing fire extinguishers
demo_video = "Fire_Video.mp4"  # Replace with a demo video containing fire extinguishers

st.title('Fire Extinguisher Detection')
...
if app_mode == 'About App':
    st.subheader("About")
    st.markdown("<h5>This is the Fire Extinguisher Detection App created with custom-trained models using YOLOv5</h5>",
                unsafe_allow_html=True)

    st.markdown("- <h5>Select the App Mode in the Sidebar</h5>", unsafe_allow_html=True)
    st.image("Images/first_1.png")
    st.markdown("- <h5>Upload the Image and Detect Fire Extinguishers in Images</h5>", unsafe_allow_html=True)
    st.image("Images/second_2.png")
    st.markdown("- <h5>Upload the Video and Detect Fire Extinguishers in Videos</h5>", unsafe_allow_html=True)
...
if app_mode == 'Run on Image':
    st.subheader("Detected Fire Extinguisher:")
    text = st.markdown("")
